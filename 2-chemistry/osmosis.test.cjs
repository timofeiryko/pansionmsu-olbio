...
for (const hydrated of [true, false]) {
  ...
  if (!hydrated) for (const p of model.particles) p.hydrating = false;
  let maxOverlap = 0, maxBound = 0;
  ...
  if (hydrated) assert.ok(maxBound>=4 && model.hydrationExchanges>30, 'Hydration shells must exchange their water');
  console.log(JSON.stringify({hydrated,seconds:60,forward:model.forward,backward:model.backward,net:model.forward-model.backward,
    concentration:Math.round(100*150/(150+model.offset)),maxOverlap,maxBound,exchanges:model.hydrationExchanges}));
}
console.log('Conservation, bidirectional pores, collisions, dilution, dynamic hydration and osmosis without hydration: OK');
